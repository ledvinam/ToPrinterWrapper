    ...
    public class ToPrinterTimeoutCancellationTests
    {
       [Fact]
        public async Task PrintDocumentAsync_ShouldRespectTimeout()
        {
            ...
            string arguments = $"-options alerts:no silent:yes log:no -props spjob:yes";
            var sw = System.Diagnostics.Stopwatch.StartNew();
            var timeout = TimeSpan.FromSeconds(1);
            await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
            {
                await printer.PrintDocumentAsync(Tests.TestFilePath, Tests.TestPrinterName, arguments, timeout, CancellationToken.None);
            });
            sw.Stop();
        ...
        public async Task PrintDocumentAsync_ShouldRespectCancellationToken()
        {
            ...
            await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
            {
                await printer.PrintDocumentAsync(Tests.TestFilePath, Tests.TestPrinterName, arguments, cancellationToken: cts.Token);
            });
            sw.Stop();
            Assert.InRange(sw.Elapsed.TotalSeconds, 0.5, 3); // Should cancel close to 1s
        }
